...
class GenericWorld:
        ...
        for bomb in self.bombs:
            if bomb.timer <= 0:
                ...
                self.explosions.append(
                    Explosion(blast_coords, screen_coords, bomb.owner, s.EXPLOSION_TIMER))
                bomb.active = False
            else:
                # Progress countdown
                bomb.timer -= 1
                ...
                for a in self.active_agents:
                    if (not a.dead) and (a.x, a.y) in explosion.blast_coords:
                        ...
                        if a is explosion.owner:
                            self.logger.info(
                                f'Agent <{a.name}> blown up by own bomb')
    ...
    def end_round(self):
        if not self.running:
            raise ValueError(
                'End-of-round requested while no round was running')
    ...
    def time_to_stop(self):
        ...
        if (len(self.active_agents) == 1
            ...
            self.logger.info(
                f'One agent left alive with nothing to do, wrap up round')
            return True
    ...
    def end(self):
        ...
        results = {'by_agent': {
            a.name: a.lifetime_statistics for a in self.agents}}
        for a in self.agents:
...
class BombeRLeWorld(GenericWorld):
    def __init__(self, args: WorldArgs, agents):
        ...
        self.total_scores = [[] for i in range(len(self.agents))]

        self.filename = ""
        for a in self.agents:
            self.filename += a.name + "_"

        self.filename = self.filename.replace("rule_based_agent", "rba")
        self.filename = self.filename.replace("noPlanMan-DQN", "DQN")
        self.filename = self.filename.replace("noPlanMan-PPO", "PPO")
        self.filename = self.filename.removesuffix("_")
        self.filename += ".png"

        self.filepath = os.path.join("Results", self.filename)
    ...
    def build_arena(self):
        ...
        arena[self.rng.random((s.COLS, s.ROWS)) <
              scenario_info["CRATE_DENSITY"]] = CRATE
        ...
        start_positions = [(1, 1), (1, s.ROWS - 2),
                           (s.COLS - 2, 1), (s.COLS - 2, s.ROWS - 2)]
        ...
        coins = []
        all_positions = np.stack(np.meshgrid(
            np.arange(s.COLS), np.arange(s.ROWS), indexing="ij"), -1)
        ...
        for exp in self.explosions:
            if exp.is_dangerous():
                for (x, y) in exp.blast_coords:
                    explosion_map[x, y] = max(
                        explosion_map[x, y], exp.timer - 1)
        ...
        for a in self.agents:
            if a.train:
                ...
                for enemy in self.active_agents:
                    if enemy is not a:
                        # pass
                        a.process_enemy_game_events(
                            self.get_state_for_agent(enemy), enemy)
        for a in self.agents:
            if a.train:
                if not a.dead:
    ...
    def plot_gameplay_results(self, agents):

        for index, a in enumerate(agents):
            self.total_scores[index].append(a.total_score)
            plt.plot(self.total_scores[index], label=a.display_name)

        plt.xlabel('Rounds')
        plt.ylabel('Accumulated Agent Scores')
        plt.title(
            f"Agent points over rounds in scenario '{self.args.scenario}'")
        plt.legend()
    ...
    def end_round(self):
        ...
        # # Plot gameplay results for the current round
        self.plot_gameplay_results(self.agents)
...
class GUI:
    ...
    def render(self):
        ...
        for bomb in self.world.bombs:
            bomb.render(
                self.screen, s.GRID_OFFSET[0] + s.GRID_SIZE * bomb.x, s.GRID_OFFSET[1] + s.GRID_SIZE * bomb.y)
        ...
        for i, a in enumerate(agents):
            bounce = 0 if (a is not leading or self.world.running) else np.abs(
                10 * np.sin(5 * time()))
        ...
        if not self.world.running:
            x_center = (s.WIDTH - s.GRID_OFFSET[0] - s.COLS *
                        s.GRID_SIZE) / 2 + s.GRID_OFFSET[0] + s.COLS * s.GRID_SIZE
            ...
            leading_total = max(self.world.agents, key=lambda a: (
                a.total_score, a.display_name))
        ...
        if self.world.running and self.world.args.make_video:
            self.world.logger.debug(
                f'Saving screenshot for frame {self.frame}')
            pygame.image.save(self.screen, str(
